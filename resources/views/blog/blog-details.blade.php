                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8">
                        ...
                        @forelse($blog->comments()->latest()->get() as $comment)
                            <div class="comment-item group bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-700 dark:to-gray-800 rounded-xl p-6 hover:shadow-xl transition-all duration-300 border-l-4 border-blue-500" data-comment-id="{{ $comment->id }}">
                                <div class="flex items-start space-x-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between mb-2">
                                            <div>
                                                <h4 class="font-bold text-gray-900 dark:text-white text-lg">{{ $comment->user->name ?? 'Anonymous' }}</h4>
                                                <p class="text-sm text-gray-500 dark:text-gray-400 flex items-center">
                                            ...
                                            @auth
                                                @if ($comment->user_id === auth()->id())
                                                    <button onclick="deleteComment({{ $comment->id }})" class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="flex items-center space-x-6">
                                            ...
                                            <button onclick="toggleCommentLike({{ $comment->id }})" 
                                                ...
                                                <svg class="w-5 h-5 group-hover/like:scale-125 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                                </svg>
                                                <span class="text-sm font-semibold comment-like-count">{{ $comment->likes()->count() }}</span>
                                            </button>
                                        ...
                                        @auth
                                            <div id="replyForm{{ $comment->id }}" class="hidden mt-4">
                                                <form action="{{ route('frontend.blog.comment.reply', [$blog->slug, $comment->id]) }}" method="POST" class="space-y-3">
                                                    @csrf
                                                        ...
                                                        <div class="flex items-start space-x-3">
                                                            ...
                                                            <div class="flex-1">
                                                                ...
                                                                <button onclick="toggleCommentLike({{ $reply->id }})" 
                                                                        class="mt-2 flex items-center space-x-1 text-gray-500 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 transition-colors">
                        ...
                        @empty
                            <div class="text-center py-16">
                                ...
                                <h3 class="text-2xl font-bold text-gray-400 dark:text-gray-500 mb-2">No comments yet</h3>
